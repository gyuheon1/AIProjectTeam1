...
origins = ["http://192.168.0.212:80", "http://localhost:80", "http://127.0.0.1:80"]
app.add_middleware(
    CORSMiddleware,
...
model = YOLO('yolo11n_best.pt')  # YOLOv8 모델 로드 (yolov8n.pt 모델의 가중치파일)
...
def detect_objects(image: Image):
    ...
    for result in results:
        ...
        for box, confidences, class_ids in zip(boxes, confidences, class_ids):
            ...
            cv2.putText(img, f'{label} {confidences:.2f}', (x1, y1), cv2.FONT_HERSHEY_PLAIN, 1, (255,0,0), 2, 2)
